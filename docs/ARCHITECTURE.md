...
2. Заливки блоков-рамок (**EditorRegionBlock**, см. §6.1) — под текстом
3. Текст по **AttributedRun** (`GlyphCache`, моноширинный/пропорциональный шрифт)
4. Inlay hints (виртуальные аннотации)
5. Selection
6. Рамки блоков-рамок — поверх выделения
7. Carets
8. Diagnostics / link underline
...
- **Вложенные блоки не режутся**: если один `range` полностью содержит другой, внешний блок остаётся целым, внутренний рисуется поверх. Заливки рисуются до текста; рамки — после selection, но до кареток; внешние (более длинные `range`) рисуются последними.
